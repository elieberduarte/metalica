...
from nucleo import analise, cargas, nbr8800, nbr14762, perfis_fabrica
...
def _verificar_frio(perfil: Perfil, aco: str, Nc: float, Nt: float, M: float,
...
def _verificar_laminado(perfil: Perfil, aco: str, Nc: float, Nt: float, M: float,
                        Lx: float, Ly: float, n: int, elemento: str, avisos: List[str]) -> Resultado:
    a = mat.aco(aco)
    Nc, Nt, M = abs(Nc) / n, abs(Nt) / n, abs(M) / n
    cant = perfil.tipo == "L"
...
def _verificar_membro(perfil: Perfil, aco: str, Nc, Nt, M, Lx, Ly, n, elemento, avisos) -> Resultado:
    try:
        if perfis_fabrica.tipo_de_verificacao(perfil) == "frio":
            return _verificar_frio(perfil, aco, Nc, Nt, M, Lx, Ly, n, elemento)
        return _verificar_laminado(perfil, aco, Nc, Nt, M, Lx, Ly, n, elemento, avisos)
    except ErroDeDados as exc:
...
def _serializar_resultado(r: Resultado) -> dict:
