...
try:
    ...
    aba.avaliar("window.editor._fecharMenus(); 1")
    # diálogo Vistas 2D: nome existente → substituir marcado (antes do clique no desenho,
    # que agora troca de tela na mesma janela)
    ...
    aba.avaliar("document.querySelector('dialog[open]').close('cancelar'); 1"); aba.drenar(0.5)
    # clicar no desenho do menu troca para o CAD na mesma janela (location.href, sem window.open)
    aba.avaliar("window.__aberto = null; window.open = (u) => { window.__aberto = u; return { focus() {} }; }; 1")
    aba.avaliar("document.querySelector('.menu[data-menu=\"desenho2d\"] .menu-botao').click(); 1")
    t0 = time.time()
    while time.time() - t0 < 10 and not aba.avaliar("!!document.querySelector('[data-desenho=\"corte-tesoura\"]')"): aba.drenar(0.3)
    aba.avaliar("document.querySelector('[data-desenho=\"corte-tesoura\"]').click(); 1")
    t0 = time.time()
    while time.time() - t0 < 30 and not aba.avaliar("location.pathname === '/cad'"): aba.drenar(0.5)
    u = aba.avaliar("location.pathname + location.search")
    ok(u == "/cad?projeto=compressores&desenho=corte-tesoura", f"clicar abre o CAD direto no desenho, na mesma janela: {u}")
    t0 = time.time()
    while time.time() - t0 < 60 and not aba.avaliar("document.body.dataset.pronto === '1' && !!window.cad && window.cad.doc.tamanho > 100"): aba.drenar(0.5)
    ok(aba.avaliar("window.cad && window.cad.nomeDesenho") == "corte-tesoura" and aba.avaliar("document.title") == "Corte tesoura — Desenho 2D",
       "CAD abriu o desenho: %s" % aba.avaliar("document.title"))
