...
try:
    ...
    aba.avaliar("window.editor._listarDesenhosNoMenu(); 1")
    # a lista vem do servidor: consulta até aparecer (2 s fixos não bastavam com a máquina ocupada)
    t0 = time.time()
    while time.time() - t0 < 30 and aba.avaliar("document.querySelectorAll('.desenhos-salvos [data-excluir]').length") != 1: aba.drenar(0.5)
