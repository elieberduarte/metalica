...
try:
    ...
    ok(aba.avaliar("document.querySelectorAll('#arquivos-corpo a').length") >= 7, "aba Arquivos com os links (CSV, HTML, PDF)")
    ok(aba.avaliar("[...document.querySelectorAll('#arquivos-corpo a')].some(a => a.href.endsWith('plano-de-corte.csv'))"), "plano-de-corte.csv na aba Arquivos")
    ok(aba.avaliar("document.querySelectorAll('#abas button[data-aba]').length") == 10, "dez abas")
    aba.avaliar("document.querySelector('#abas [data-aba=corte]').click(); 1")
    nb = aba.avaliar("document.querySelectorAll('#aba-corte .barra-corte').length")
    ns = aba.avaliar("document.querySelectorAll('#aba-corte .barra-corte .seg').length")
    ok(nb >= 10 and ns > nb, "aba Plano de corte com as barras desenhadas: %s linhas de corte, %s peças" % (nb, ns))
    foto(aba, "_materiais_corte.png")
    aba.avaliar("document.querySelector('#abas [data-aba=geral]').click(); 1")
    ok(aba.avaliar("document.querySelectorAll('#aba-geral .barras-peso .trilho').length") >= 3, "visão geral com as barras de peso por categoria")
    ok("aço" in (aba.avaliar("document.querySelector('#cartoes').textContent") or ""), "cartão do aço separado das telhas")
    foto(aba, "_materiais.png")
    aba.avaliar("document.querySelector('#abas [data-aba=romaneio]').click(); 1")
    ok(aba.avaliar("!document.querySelector('#aba-romaneio').hidden && document.querySelector('#aba-geral').hidden"), "aba Romaneio aberta")
    ...
    # resumos da obra: pela aba, formulário ao lado e a prévia na própria página
    ...
    src = aba.avaliar("(document.querySelector('#doc-previa iframe') || {}).src || ''")
    ok("resumo-da-obra.html" in src, "prévia do resumo da obra na página: %s" % src[-60:])
    ok(aba.avaliar("!!document.querySelector('#doc-previa a[href$=\"resumo-da-obra.pdf\"]')"), "botão Abrir o PDF")
    ok(aba.avaliar("window.__abertos.length") == 0, "nada abre sozinho em outra janela")
    aba.drenar(2.0)
    foto(aba, "_materiais_resumos.png")
    aba.avaliar("[...document.querySelectorAll('#doc-previa .alternar button')][1].click(); 1")
    aba.drenar(1.0)
    ok("resumo-de-materiais.html" in (aba.avaliar("document.querySelector('#doc-previa iframe').src") or ""), "alterna para o resumo de materiais")
    ok(aba.avaliar("[...document.querySelectorAll('#arquivos-corpo a')].some(a => a.href.endsWith('resumo-da-obra.pdf'))"), "os resumos aparecem na aba Arquivos")
